...
impl SqliteClipboardRepository {
    ...
    /// A recaptured clipboard value may reuse an ID, but it does not own user metadata.
    /// Keep the row and its tag index in one transaction before enforcing history limits.
    pub fn save_capture_with_conn(
        &self,
        conn: &Connection,
        ...
        if entry.id > 0 {
            let existing = self.get_entry_by_id_with_conn(&transaction, entry.id)?
                .ok_or_else(|| "Clipboard entry no longer exists".to_string())?;
            entry.preserve_capture_metadata(&existing);
        }
        let id = self.save_with_conn(&transaction, entry, data_dir)?;
        let saved = self.get_entry_by_id_with_conn(&transaction, id)?
            .ok_or_else(|| "Saved clipboard entry is missing".to_string())?;
...
impl ClipboardRepository for SqliteClipboardRepository {
    ...
    fn clear(&self, data_dir: Option<&std::path::Path>) -> Result<(), String> {
        ...
        // Delete one-by-one to clean up each entry's managed attachments.
        for id in &ids {
            self.delete_with_conn(&conn, *id, data_dir)?;
        }
